...
- **Spinner and Toaster forward native contracts.** Spinner accepts SVG props/refs while keeping localized naming, decorative behavior, sizes and the reveal delay. Toaster accepts Sonner props/refs and merges partial icons/styles/classNames with Ledger defaults. Mobile stacks respect native offsets in RTL without overflowing the viewport. Live, independently targeted toast stories replace copied markup and singleton bookkeeping. Current shadcn Toast docs also offer Base UI Toast; switching the toast manager remains separate from this Sonner cleanup.
