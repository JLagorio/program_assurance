...
/*
 * The frame before the navigation system: a sidebar of objects and queues, a top bar, the page.
 * Deprecated. It stays so a consumer moves to Shell.SideNav, Shell.TopNav and Shell.Main in a
 * commit of its own; the lint names the replacement for each part. Nothing new goes here.
 */
...
function Sidebar({
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-layout-sidenav shrink-0 flex-col border-e border-default bg-surface-sunken lg:flex">
      {brand ? (
        <div className="flex h-layout-topnav items-center gap-150 px-200">{brand}</div>
      ) : null}
...
function Brand({
  ...
  return onClick ? (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-150 rounded-medium text-left outline-none focus-visible:outline-focused"
    >
      {inner}
    </button>
  ) : (
...
/** @deprecated Use SideNavItemProps. */
...
function TopBar({ children, actions }: { children?: ReactNode; actions?: ReactNode }) {
  return (
    <header className="sticky top-0 z-20 flex h-layout-topnav shrink-0 items-center gap-150 border-b border-default bg-surface px-200 lg:px-300">
      {children}
...
/** @deprecated The old frame. Use Shell with Shell.TopNav, Shell.SideNav and Shell.Main. */
export const legacy = { Root: ShellRoot, Sidebar, Brand, Mark, NavGroup, NavItem, User, TopBar };
