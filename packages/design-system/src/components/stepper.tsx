...
function StepperItem({ state, label, meta, title, first, last, onSelect }: StepperItemProps) {
  const circle = (
    <span
      className={cn(
        "grid size-200 shrink-0 place-items-center rounded-full border transition-colors duration-fast ease-standard",
        marker[state],
      )}
    >
  ...
  const text = (
    <>
      ...
      {meta ? (
        <span className="truncate font-body-xsmall text-subtle tabular-nums">{meta}</span>
      ) : null}
    </>
  );
  ...
  const tagProps = onSelect
    ...
    : {};
  const rail = (kind: "h" | "v", hidden: boolean) => (
    <span
      aria-hidden
      className={cn(
        "flex-1 border-default",
        kind === "h" ? "h-0 border-t" : "w-0 border-s",
        hidden && "invisible",
      )}
    />
  );
  ...
  return (
    <li
      title={title}
      className="group/step relative flex min-w-0 flex-1 flex-col group-data-[orientation=vertical]/stepper:flex-none group-data-[orientation=vertical]/stepper:flex-row group-data-[orientation=vertical]/stepper:gap-150"
    >
      <span className="flex items-center group-data-[orientation=vertical]/stepper:hidden">
        ...
        <Tag
          ...
          className="rounded-full outline-none focus-visible:outline-focused"
        >
          {circle}
        </Tag>
        {rail("h", Boolean(last))}
      ...
      <Tag
        {...tagProps}
        className="flex min-w-0 flex-col items-center px-050 pt-075 text-center outline-none focus-visible:outline-focused group-data-[orientation=vertical]/stepper:hidden"
      >
        {text}
      </Tag>
      ...
      <Tag
        {...tagProps}
        className="hidden min-w-0 flex-col items-start pb-200 text-left outline-none focus-visible:outline-focused group-data-[orientation=vertical]/stepper:flex"
      >
        {text}
      </Tag>
    </li>
