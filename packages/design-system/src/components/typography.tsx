...
export function Eyebrow({ children, tone = "neutral", className }: { children: ReactNode; tone?: Tone | undefined; className?: string | undefined }) {
  return <div className={cn("font-heading-xxsmall uppercase", tone === "neutral" ? "text-subtlest" : toneClasses[tone].text, className)}>{children}</div>;
}
...
export function Prose({ label, tone = "neutral", children, className }: { label: string; tone?: Tone | undefined; children: ReactNode; className?: string | undefined }) {
  return (
...
/** Inline `label value` pair for the facts strip under a record header. Renders dt/dd; wrap a row of them in a <dl>. */
