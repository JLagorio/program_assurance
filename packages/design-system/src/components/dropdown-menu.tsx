...
function DropdownMenuRoot({ trigger, align = "start", width = 200, defaultOpen = false, children }: DropdownMenuProps) {
...
function DropdownMenuItem({ children, isSelected, disabled, onSelect, trailing }: { children: ReactNode; isSelected?: boolean | undefined; disabled?: boolean | undefined; onSelect?: (() => void) | undefined; trailing?: ReactNode }) {
  return (
    <DropdownMenuPrimitive.Item {...(disabled ? { disabled } : {})} onSelect={() => onSelect?.()} className={cn(menuItem, isSelected ? menuItemSelected : menuItemHighlighted, menuItemDisabled)}>
...
function DropdownMenuLabel({ children }: { children: ReactNode }) {
  return <DropdownMenuPrimitive.Label className={menuLabel}>{children}</DropdownMenuPrimitive.Label>;
}
...
export const DropdownMenu = Object.assign(DropdownMenuRoot, { Item: DropdownMenuItem, Label: DropdownMenuLabel, Separator: DropdownMenuSeparator });
