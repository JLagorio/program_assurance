...
Ledger owns semantic contracts: `tone` communicates meaning; `variant` chooses treatment; `size` chooses a documented scale; `is…` denotes component state; `should…` opts into behavior. Native DOM properties keep native names (`disabled`, `name`, `form`, `aria-*`, `data-*`). Controlled state pairs a value with its change callback; action callbacks use the action's verb. A compound child describes its role (`Dialog.Footer`, `Editable.Text`). Do not rename a native or domain term just to make spelling uniform.

DOM attributes and refs belong on the element that consumers must label, submit, focus, measure or integrate. Composite components document that target; they do not promise that every prop reaches an arbitrary wrapper. Custom Field controls call `useFieldControl`; slotted Button children must accept their injected props and ref. Link navigation uses `Button asChild` around one anchor, never a button inside an anchor.

Radix, Vaul and layout helpers are implementation dependencies. Consumers should use Ledger props rather than importing their private internals. **Explicit public adapters** are Sonner's toast options/promise API, TanStack table definitions, and chart configuration types exposed by the package. Upgrades of these adapters require checking consumer types and migration notes. React and Tailwind remain peers. `MODE_STORAGE_KEY` and `SHELL_STORAGE_KEY` are public storage integration constants; persisted data must be validated and fall back safely.
