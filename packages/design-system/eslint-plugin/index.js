...
const generated = JSON.parse(fs.readFileSync(path.join(here, "../src/generated/utilities.json"), "utf8"));
  ...
  switch (node.type) {
    ...
    case "ObjectExpression":
      for (const p of node.properties) {
        ...
        if (p.key.type === "Literal" && typeof p.key.value === "string") out.push({ text: p.key.value, node: p.key });
        if (p.key.type === "Identifier" && !p.computed) out.push({ text: p.key.name, node: p.key });
      }
      return;
    case "CallExpression":
      if (node.callee.type === "Identifier" && CLASS_FNS.has(node.callee.name)) for (const a of node.arguments) collect(a, out);
      return;
...
function forEachClass(context, cb) {
  ...
  return {
    JSXAttribute(node) {
      if (node.name.type === "JSXIdentifier" && (node.name.name === "className" || node.name.name === "class")) handle(node.value);
    },
    CallExpression(node) {
      if (node.callee.type === "Identifier" && CLASS_FNS.has(node.callee.name)) for (const a of node.arguments) handle(a);
    },
  };
}
...
const isKnown = (base) => tokenClasses.has(base) || spacing.test(base) || structural.some((r) => r.test(base));
...
const rules = {
  ...
  "no-alpha-token": rule("A state is a token, never alpha on a base token.", (context) =>
    forEachClass(context, ({ cls, base }, node) => {
      if (/^(bg|text|icon|border|shadow|ring|outline|decoration|divide|fill|stroke|from|via|to|accent|caret)-[a-z0-9-]+\/\d+$/.test(base))
        context.report({ node, message: `"${cls}" applies alpha to a token. Use the hovered, pressed, subtle or disabled token instead.` });
    }),
  ),
  "no-dark-variant": rule("A dark: class means a token is missing.", (context) =>
    forEachClass(context, ({ cls, variants }, node) => {
      if (variants.includes("dark")) context.report({ node, message: `"${cls}" uses the dark variant. Every colour is a token that flips by itself; add the token.` });
    }),
  ),
  "no-margin": rule("Spacing between siblings comes from Stack, Inline and Bleed, not margins.", (context) =>
    forEachClass(context, ({ cls, base }, node) => {
      if ((/^-?(m|mx|my|mt|mb|ml|mr|ms|me)-/.test(base) && !/-auto$/.test(base)) || /^-?(m|mx|my|mt|mb|ml|mr|ms|me)$/.test(base))
        context.report({ node, message: `"${cls}" is a margin. Use Stack or Inline space, or Bleed.` });
    }),
  ),
  "no-static-design-value": rule("Static Tailwind utilities that encode a design value the tokens own.", (context) =>
    forEachClass(context, ({ cls, base }, node) => {
      const hit =
  ...
  "no-non-token-class": rule("Every class is a token utility or a structural utility from the documented list.", (context) =>
    forEachClass(context, ({ cls, base }, node) => {
      if (/^\[|-\[/.test(base)) return; // no-arbitrary-value reports these
      if (!isKnown(base)) context.report({ node, message: `"${cls}" is neither a token utility nor a documented structural utility.` });
    }),
  ),
  "no-deprecated-token": rule("A deprecated token, with its replacement.", (context) =>
    forEachClass(context, ({ cls, base }, node) => {
      ...
      if (dep)
        context.report({
          ...
          fix: dep.replacementClass && node.type === "Literal" ? (fixer) => fixer.replaceText(node, JSON.stringify(node.value.replace(base, dep.replacementClass))) : null,
        });
    }), { fixable: "code" },
  ),
  "use-primitives": rule("Layout in product code goes through Box, Stack, Inline, Flex and Grid.", (context) => ({
    JSXOpeningElement(node) {
      if (node.name.type !== "JSXIdentifier" || !/^(div|span|section|article|aside|header|footer|main|nav|ul|ol|li|form|fieldset)$/.test(node.name.name)) return;
      ...
      if (/(^|\s)(flex|inline-flex|grid|inline-grid)(\s|$)/.test(text) || /(^|\s)-?(p|px|py|pt|pb|ps|pe|gap|gap-x|gap-y)-/.test(text))
        context.report({ node, message: `<${node.name.name}> carries layout classes. Use Box for padding and background, Stack or Inline for a row or column, Flex or Grid otherwise.` });
    },
  })),
};
...
const portability = {
  "no-restricted-imports": [
    ...
    {
      patterns: [
        { group: ["@/*"], message: "The package imports nothing from a consumer. Use a relative path inside the package." },
        { group: ["**/src/**", "../../../src/*"], message: "The package imports nothing from the prototype." },
        { group: ["@tanstack/react-router", "@tanstack/react-router/*", "react-router*", "next/link"], message: "No router dependency. A link is a slot the consumer fills (asChild)." },
      ],
    },
  ],
};
