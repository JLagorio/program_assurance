...
The **New program** action opens a dedicated five-step walkthrough: program details, catalog and profiles, systems and subsystems, categorization and tailoring, then review and create. It saves the complete setup in one Supabase transaction and opens the new program. See [the program wizard guide](docs/program-wizard.md) for supported OSCAL tailoring and the records it creates.
