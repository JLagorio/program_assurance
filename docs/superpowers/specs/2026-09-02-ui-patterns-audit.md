...
## Flagged later the same day: choosing many, the preview stack, the hover ladder

Josef, 2026-09-02, on requirement mapping: a Combobox will not hold up when a system has hundreds of requirements and the question is which of them apply to one element. The HubSpot association panel is the reference: the same side panel, a compact table with search, filters, sortable columns and checkboxes, and a footer that adds the selection. Three items follow, and together they are one panel with a stack of frames.

9. **Choosing many from hundreds.** Today: `Combobox` in "Allocate a requirement" (system-tree.tsx), two more in the tailoring pane, `RecordPicker` (a `Command.Dialog`) for evidence, and `ApplicabilityModal` walking obligations one at a time. The kit's Pickers story covers one record, known by name. It has nothing for "many, chosen by attribute". Recommendation: **kit**, as `PickerSheet`: a `Sheet` whose body is a search `Input`, `FilterChip`s, a `Table` with sortable `Table.Header` and `Table.Selection`, and a sticky footer reading "Allocate 12 to Flight computer". Precedent: HubSpot associations, Airtable "link to record", Salesforce lookup. The per-row fields the model requires (responsibility, coverage, the bounded claim) go in a second frame of the same panel: the selected rows as a compact table with `Editable` cells and a default applied to all, never a form per row. "Does not apply, because" is a row action in the same table, so the applicability walk becomes a mode of the picker rather than a separate dialog. Where this serves: requirements onto an element (tree kebab), elements onto a requirement (coverage), controls into a set (tailoring), controls that derive requirements.
10. **The preview stack and a compact record header.** `PreviewSheet` previews one record and `NodePreviewSheet` already drills into a child in place, but with no way back; a control clicked inside a preview leaves the panel. Recommendation: **kit**. `PreviewSheet` gains `back`/`onBack` and its header becomes the compact form of `RecordHeader` (id, title, meta, at most three facts, one status), so a record reads the same in the panel and on its page. The stack itself belongs in the URL (`?peek=CN-0300,AC-2` as a search param): the panel's back chevron and the browser's back button are then the same thing, a peek is linkable, and close drops the whole stack. Precedent: Linear side peek, Notion peek, HubSpot's record sidebar. Frames on the stack: a record preview, a picker (item 9), another record preview.
11. **The hover ladder.** `HoverCard` exists (Radix, 400ms, opens on focus) and is used on program and risk ids. Recommendation: **use it, write the rule**. Three rungs: hover on an id or name = a glance, facts only, no actions; click = the preview sheet, facts and the actions that make sense without leaving; the footer link = the record, where the work is done. One preview body per record type at two densities (glance, peek), rendered by the kit's three shells (`HoverCard`, `PreviewSheet`, `PreviewRail`). Today `ProgramPeek`, `RiskPeek` and `NodePreviewSheet` are three unrelated bodies; that is the app's to unify, not the kit's.
