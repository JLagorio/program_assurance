...
- **Hydration.** The app renders on the server, so the default layout is what the server sends and the stored view applies after mount, in one commit. Widths and order can shift once on first paint; the setting for density does not, because it rides the mode switch's before-paint script. The drag context takes a `useId` so its generated ids match between server and client.
...
2. **Choosing.** Selection, the bar, select-all-pages, row actions, column filters on facets, presets. PickerSheet on it. Stories: Selection, Row actions, Filtering, Presets. *Landed 2026-09-03 as `DataTable.SelectionBar`, `.Filter`, `.Search` and `.Presets`, with one filter function, `matches`, behind the facet kinds (an array is membership, a string is equality, `{ contains }` is a substring) and `dateRange` behind dates. The risks and programs routes own their filters in one `columnFilters` state: the tabs write the status filter, the chips write theirs. Preset counts are of every row, before search. PickerSheet's story is still by hand; it moves with step 6, where its second frame needs the editable cells.*
3. **Columns.** Pinning with offsets and the edge shadow, resizing, hiding, column groups, reordering by drag and keyboard, and the reader's view in `localStorage` with Reset view. Stories: Pinned columns, Resizing, Column choice, Column groups, Reordering columns, Saved view. *Landed 2026-09-03. `Table` itself is the scroll frame now (no separate `Table.Scroll`): it carries `data-scrolled-start`/`-end` and the pinned edge is a border that turns bold once the frame scrolls, since the kit has no edge shadow token. `Table.Header` and `Table.Cell` take `pinned`, `offset` and `edge`; `Table.Header` takes `resize` and `trailing`. The selection column pins with the start band. Reordering is dnd-kit with a grip on hover, pointer and keyboard; pinned columns keep their band. The column menu (sort, pin, hide) sits on every header's hover; `DataTable.Columns` shows and hides and ends with Reset view. A table that resizes or reorders takes the fixed layout, sized from its columns, so the browser cannot second-guess a width; the unsized column takes the slack. The view store is one hook inside `useDataTable`; the URL still owns sort, filters and page.*
4. **Rows.** Tree mode with the treegrid keyboard, detail rows, groups on the grouped row model, pinned rows and totals, row reordering with the handle. System tree on it. Stories: Tree, Detail rows, Groups, Pinned rows, Totals, Reordering rows. *Landed 2026-09-03. `tree`, `detail`, `groupBy`, `pinRows`, `footer` on a number column and `reorderRows` on the hook; `Table.Detail` and `Table.Handle` in the kit; one drag context serves columns and rows. The tree keeps the reader's open rows across a data refresh (`autoResetExpanded` off), which the system tree needs because its projection is rebuilt per store change. Groups turn pagination off; row reordering turns sorting off. The system tree is on tree mode: the columns are custom kinds over the projection, the row menu is `c.actions`, the preview opens on row click. Pinned rows and totals share one story.*
5. **Scale.** Virtualization, server mode, the URL adapter, `toRows`. Stories: Virtualized, Server. *Landed 2026-09-03. `virtualize` on the hook, on `@tanstack/react-virtual` with the frame as the scroll element and the density's row height as the estimate: ten thousand rows draw about twenty. `manual` and `rowCount` are TanStack's own; the Server story fakes one. `toRows` and `toCsv` in the kit; the risks Export button downloads the CSV. `src/lib/table-state.ts` is back as the URL adapter: `validateTableSearch` for a route and `useTableSearch` binding sort, page, search and filters to the search params; the risks route is on it, so a link carries the question and the back button answers it.*
6. **Editing.** `Editable` cells with the column keyboard model, the grid role when a table is editable. PickerSheet frame two on it. Story: Editing. *Landed 2026-09-03. `editable` on the text and status kinds (`onChange`, `save`, `validate`, a status's `options`), drawn as `Editable.Text` and `Editable.Select`; the table is a `grid` when any column edits; Enter commits and moves down the column, Tab keeps its meaning and moves across (a departure from the plan, which had both move down: Tab across is what every sheet does). Both PickerSheet frames are DataTables now: frame one with search, the family and state facets and a selection kept by row id, frame two with the fields in place and "Does not apply" per row.*
...
All six steps landed on 2026-09-03, in seven commits on `claude/wakey-wakey-atwe12`, each with its stories, its docs page and a browser drive. What the plan said and what changed is in the italics under each step. Three things are left for the next reader of this spec: the coverage view, the control board and the other seventy-odd registers still draw with `Table` and move when touched; virtualization and detail rows do not combine (a detail row's height is not the row height); and a text column with more than thirty distinct values filters by substring rather than by checkbox.
