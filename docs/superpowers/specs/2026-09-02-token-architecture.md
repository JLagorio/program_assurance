...
7. **Mode switch.** Landed 2026-09-02, evening: `src/mode` exports `ModeProvider`, `useMode`, `ModeSwitch` (a ToggleGroup: light, dark, system; icons only in chrome, `showLabels` in a form; `value`/`onChange` override the provider), `readMode`/`writeMode`/`applyMode` on `localStorage["ledger.color-mode"]`, and `modeScript` (with `modeScriptFor(key)`) for the document head. The prototype renders the script from the root route's `head()`, wraps the app in the provider, puts the switch in the top bar, and the `data-color-mode="light"` pin is gone; a choice survives a reload with no flash. Docs at Components/Mode switch.
...
8c. **Audit items 9–11.** Landed 2026-09-02, evening: `Sheet` gains `onBack`, `facts` and a non-scrolling `toolbar`; `PreviewSheet` gains `onBack`, `status` and `facts` (the compact RecordHeader: eyebrow with id and one status, title, meta, at most three facts); `PickerSheet` in patterns (spec: `2026-09-02-picker-sheet.md`); the hover ladder and "Choosing many" are written in Patterns.mdx. The prototype adoptions (the picker in "Allocate a requirement", the peek stack in the URL) are the other agent's.

8b. **Audit batch.** Landed 2026-09-02, evening, from the spine audit (`2026-09-02-ui-patterns-audit.md`, decisions table at its end): `TextLink`, `PreviewSheet` (and a Sheet `eyebrow` slot), `Table.Tree`, `Table.Id width`, `ToggleGroup` item `count`, `Fact.Group` and `RecordHeader facts`; lint rules `prefer-text-link` and `no-colgroup` in both presets; the prototype swept onto all of them (147 links, 119 colgroups, the five spine adoptions). Josef's note on forms: `controlBase` gains `aria-[invalid=true]:border-danger` and `Field` puts `aria-invalid` on the control it wraps; the app's forms still have to pass `isRequired` and `error`. Josef: no side-by-side modes in the matrices, the toolbar switches; the `bothModes` decorator is gone. The living list from here is `docs/next.md`.

9. **Publishable build.** Landed 2026-09-02, late, ahead of the second project because Josef said keep going: `npm run build` in the package (`build/dist.mjs`) runs `tsc -p tsconfig.build.json` into `dist/` (88 ESM modules with declarations and source maps, the stories excluded) and copies the stylesheets and token data beside them. The export map carries three conditions: `development` → `src/index.ts` (the workspace: Vite dev, the root `tsc` through `customConditions`, both Storybooks), `types` → `dist/index.d.ts`, `default` → `dist/index.js` (a production build, a published consumer). The CSS entries stay on `src` because `ledger.css` scans `../` for classes and the source ships in the tarball. The root `npm run build` builds the package before the app, so the prototype's production build consumes dist every time; `npm pack` runs the build through `prepack`. Proven twice: Node resolves the two conditions to the two files, and the throwaway Vite consumer installs the tarball, resolves dist, and its output carries the kit's classes.
