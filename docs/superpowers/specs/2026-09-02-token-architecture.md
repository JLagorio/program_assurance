...
8. **Prototype cutover.** Step one landed 2026-09-02: `src/ds` is a façade over the package (four barrels re-export it; adapters keep the old `items`/`to`/`backTo` link dialects until call sites are rewritten; the product frame is composed from `Shell.*` with the nav data and the router in `src/ds/shell/shell.tsx`); the old kit source and its prototype stories are deleted; `styles.css` imports `ledger.css` and `base.css`; the root pins `data-color-mode="light"`; an AST codemod renamed the kit props at the call sites; five old theme colours that collided with package utilities (`success`, `danger`, `warning`, `subtle`, `input`) are prefixed `legacy-` in the old `tokens.css` and at their call sites. Remaining: rewrite the adapter call sites (24 Tabs, 5 Breadcrumb, 23 RecordHeader, 5 Item) and delete the adapters; burn down the raw Tailwind classes route by route (about 1,800 sites, 307 of them `legacy-*`), then import `reset.css` and delete the old `tokens.css`; adopt the lint preset in the root ESLint config; move imports from `@/ds/*` to the package and delete the façade.
