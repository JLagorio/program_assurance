...
export function restoreCompositionChanges() {
  ...
  const raw = workspaceStorage.getItem(compositionStorageKey);
  const next = raw
...
function persist(next: Snapshot) {
  if (typeof window === "undefined") throw new Error("System changes require an open workspace.");
  try {
    workspaceStorage.setItem(compositionStorageKey, JSON.stringify(next));
  } catch (error) {
    throw new Error(
      `System changes could not be saved. ${error instanceof Error ? error.message : "Check workspace storage and try again."}`,
    );
  }
}
