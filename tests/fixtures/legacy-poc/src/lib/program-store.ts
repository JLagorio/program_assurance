...
export function restorePrograms() {
  ...
  const raw = workspaceStorage.getItem(programKey);
...
export function restoreProgramCommands() {
  ...
  restorePrograms();
  const raw = workspaceStorage.getItem(commandKey);
  if (!raw) {
    restored = true;
    return;
  }
...
export function saveProgramCommands(ids: string[], patch: CommandPatch) {
  ...
  if (typeof window !== "undefined") workspaceStorage.setItem(commandKey, JSON.stringify(next));
  commandPatches = next;
  for (const id of ids)
    Object.assign(
  ...
  useEffect(() => {
    try {
      ...
      toast.add({
        ...
        type: "error",
        timeout: 8000,
        description: "Workspace storage is unavailable or saved data is invalid.",
      });
    }
  }, []);
...
export function addProgram(input: Omit<Program, "id"> & { id?: string }): Program {
  ...
  if (typeof window !== "undefined") workspaceStorage.setItem(programKey, JSON.stringify(next));
  savedPrograms = next;
