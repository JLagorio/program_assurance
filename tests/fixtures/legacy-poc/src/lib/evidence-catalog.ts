...
export function restoreEvidence() {
  ...
  const raw = workspaceStorage.getItem(storageKey);
  if (raw) {
...
function commit(artifact: EvidenceArtifact, persistRelated?: () => void) {
  ...
  const previous = typeof window !== "undefined" ? workspaceStorage.getItem(storageKey) : null;
  if (typeof window !== "undefined")
    workspaceStorage.setItem(storageKey, JSON.stringify([...next.values()]));
  try {
    // A control link and its audit entry must both save before either store publishes it.
    persistRelated?.();
  } catch (error) {
    if (typeof window !== "undefined") workspaceStorage.setItem(storageKey, previous ?? "[]");
    throw error;
  }
