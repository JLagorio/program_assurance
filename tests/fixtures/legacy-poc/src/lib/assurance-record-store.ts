...
export function restoreAssuranceRecords() {
  ...
  const raw = workspaceStorage.getItem(assuranceStorageKey);
  if (raw) {
...
function commit(changes: Partial<SavedRecords>) {
  ...
  if (typeof window === "undefined") throw new Error("Saving records requires an open workspace.");
  workspaceStorage.setItem(assuranceStorageKey, JSON.stringify(next));
  saved = next;
