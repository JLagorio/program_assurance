...
export function restoreLibrary() {
  ...
  const raw = workspaceStorage.getItem(libraryStorageKey);
...
function transaction(edit: (next: LibraryState) => void) {
  restoreLibrary();
  if (typeof window === "undefined") throw new Error("Catalog changes require an open workspace.");
  const next = clone(state);
  edit(next);
  const parsed = stateSchema.parse(next) as LibraryState;
  validate(parsed);
  try {
    workspaceStorage.setItem(libraryStorageKey, JSON.stringify(parsed));
  } catch (error) {
    throw new Error(
      `Changes could not be saved. ${error instanceof Error ? error.message : "Check workspace storage and try again."}`,
    );
  }
  publish(parsed);
}
