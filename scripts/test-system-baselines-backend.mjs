...
select pg_temp.expect((select e.source_system_id=${q("child")} and e.inherited and p.state='draft' from public.system_effective_baselines e join public.profile_resolutions p on p.id=e.profile_resolution_id where e.system_id=${q("grandchild")}),'Tailored child baseline is explicit, draft and inherited by its descendants');
...
console.log(
  "Baseline rollback passed: exact OSCAL selection, inheritance/boundaries, CAS/retries, access and scope/SSP fallback.",
);
