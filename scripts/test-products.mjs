...
async function saveElement() {
  await dialog().getByRole("button", { name: "Save element", exact: true }).click();
...
async function newConfiguration(code, name, description) {
  await page.getByRole("button", { name: "New configuration", exact: true }).first().click();
...
async function newElement(name, code, type, untick = []) {
  await page.getByRole("button", { name: "Add element", exact: true }).first().click();
  await dialog().getByRole("textbox", { name: "Name", exact: true }).fill(name);
  await dialog().getByRole("textbox", { name: "Code", exact: true }).fill(code);
...
try {
  ...
  await page
    ...
    .click();
  await dialog().getByRole("heading", { name: "Component · Audit policy", exact: true }).waitFor();
  assert.equal(
  ...
  await page.screenshot({ path: "/tmp/products-element-sheet.png", fullPage: true });
  await saveElement();
  ...
  assert.equal(await page.getByRole("button", { name: "Add element", exact: true }).count(), 0);
  ...
  await page.getByText("Variant of", { exact: true }).first().waitFor();
  ...
  assert.match(variantsText, /2 inherited · 1 added/);
  await page.screenshot({ path: "/tmp/products-variants-tab.png", fullPage: true });
  await page.goto(`${origin}/programs/${programId}/systems/${boundary.id}`);
  await page
    .getByRole("complementary", { name: "Record properties", exact: true })
